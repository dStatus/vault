...
var beakerPng = fs.readFileSync(__dirname + '/scaffold/test-static-dweb/beaker.png')
...
test('vault.readdir', async t => {
  var vault = new DWebVault(testStaticDWebURL)
...
test('vault.readFile', async t => {
  ...
  // read hex
  var beakerPngHex = await vault.readFile('beaker.png', 'hex')
  t.deepEqual(beakerPngHex, beakerPng.toString('hex'))

  // read base64
  var beakerPngBase64 = await vault.readFile('beaker.png', 'base64')
  t.deepEqual(beakerPngBase64, beakerPng.toString('base64'))

  // read binary
  var beakerPngBinary = await vault.readFile('beaker.png', 'binary')
  t.truthy(beakerPng.equals(beakerPngBinary))

  // timeout: read an vault that does not exist
  var badVault = new DWebVault(fakeDWebURL)
...
test('versioned reads and writes', async t => {
  // create a fresh dweb
  var vault = await DWebVault.create({title: 'Another Test DWeb'})
  ...
  // helper
  function checkout (v) {
    return new DWebVault(vault.url + v)
  }
...
test('vault.createNetworkActivityStream', async t => {
  ...
  var vault = new DWebVault(testStaticDWeb2URL)
  await vault._loadPromise
